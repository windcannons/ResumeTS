...
export default class extends Vue {
    // 头部开始
    private userinfo = "";
    private usertext = " 一个勤勤恳恳、认真负责的前端开发工程师，目前居住在上海。";
    private num = 0;
    ...
    //   头部结束
    //   基本信息开始
    private IsTopMove = false;
    private imgMove = false;
    private userMove = false;
    private isChangeImg = false;
    private winWidth = 0;
    //   基本信息结束
    // 求学和工作经历开始
    private scrollTitle = false;
    private scrollContent = false;
    //   求学和工作经历结束
    //   专业技能开始
    private scrollSkill = false;
    //   专业技能结束
    //   提供服务开始
    private offerTop = false;
    private offerContent1 = false;
    private offerContent2 = false;
    //   提供服务结束
    //   我的作品开始
    private myOpusTop = false;
    private myOpusContent = false;
    private promptleft = "";
    private prompttop = "";
    private moveNum = 0;
    //   我的作品结束
    //   联系我开始
    private contectTop = false;
    private contectContent = false;
    //   联系我结束
    //   回到顶部
    private isscroll = false;
    private isListBox = true;

    mounted() {
        // UserListStore.storeActionPageUserList(this.param);
        // 图片盒子旋转
        (this.$refs.riImgBox as HTMLElement).style.height = (this.$refs.riImgBox as HTMLElement).offsetWidth + 'px',
            // 我的作品
            (this.$refs.myOpusContent as HTMLElement).style.height = (this.$refs.myOpusContent as HTMLElement).offsetWidth + "px"
        // 所有页面滚动事件
        window.onscroll = () => {
            // 头部开始
            this.winWidth = window.innerWidth
            this.IsScroll = window.pageYOffset > 70;
            this.isscroll = window.pageYOffset > 400;
            if (window.pageYOffset < 260) {
                this.current = 0
            }
            // 头部结束
            //  基本信息开始
            let difference = window.pageYOffset - (this.$refs.MainBox as HTMLElement).offsetTop
            if (difference >= -801) {
                this.IsTopMove = true
            }
            if (difference >= -550) {
            ...
            // 基本信息结束
            // 求学和工作经历开始
            let experienced = window.pageYOffset - (this.$refs.experienced as HTMLElement).offsetTop
            if (experienced >= -800) {
            ...
            //   求学和工作经历结束
            //   专业技能开始
            let skill = window.pageYOffset - (this.$refs.skill as HTMLElement).offsetTop
            if (skill > -600) {
                this.scrollSkill = true
                this.current = 3
            }
            //   专业技能结束
            //   提供服务开始
            let offer = window.pageYOffset - (this.$refs.offer as HTMLElement).offsetTop
            if (offer > -600) {
                this.offerTop = true
            }
            if (offer > -400) {
                this.current = 4
                this.offerContent1 = true
            }
            if (offer > -100) {
                this.offerContent2 = true
            }
            //   提供服务结束
            //   我的作品开始
            let myOpus = window.pageYOffset - (this.$refs.myOpus as HTMLElement).offsetTop
            if (myOpus > -600) {
                this.myOpusTop = true
            }
            if (myOpus > -500) {
            ...
            if (myOpus > -300) {
                this.myOpusContent = true
            }
            //   我的作品结束
            //   联系我开始
            let contect = window.pageYOffset - (this.$refs.contect as HTMLElement).offsetTop
            if (contect > -600) {
                this.contectTop = true
            }
            if (contect > -400) {
                this.current = 6
                this.contectContent = true
            }
            //   联系我结束
        }
        // 头部开始
        let time = setInterval(() => {
            this.userinfo += this.usertext[this.num]
            this.num++
            if (this.num === this.usertext.length) {
                clearInterval(time)
            }
        }, 100)
    }
...
//   回到顶部
    ...
    listbox() {
        ...
        const appMain = this.$refs.appMain as HTMLElement;
        const appList = this.$refs.appList as HTMLElement;
    ...
    //   我的简历图片旋转
    revolve(e: MouseEvent): void {
        const ImgBox = this.$refs.ImgBox as HTMLElement;
        const Introduce = this.$refs.MainBox as HTMLElement;
    ...
    // 我的作品提示
    promptLis(e: MouseEvent): void {
        const target = e.target as HTMLElement;
        this.moveNum = parseInt(target.dataset.num || "0");
        this.promptleft = e.offsetX + "px";
        this.prompttop = e.offsetY + 25 + "px";
    }
}
</script>
